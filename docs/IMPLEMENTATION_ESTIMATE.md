...
| カテゴリ | 工数（人日） | 単価（円/人日） | 金額（円） | 比率 |
|---------|------------|---------------|-----------|------|
| フロントエンド実装 | 41.5 | 80,000 | 3,320,000 | 35.5% |
| バックエンド実装 | 42.5 | 80,000 | 3,400,000 | 36.4% |
| テスト実装 | 13.0 | 70,000 | 910,000 | 11.1% |
| デザイン・UX | 7.0 | 75,000 | 525,000 | 6.0% |
| デプロイ・インフラ | 5.0 | 70,000 | 350,000 | 4.3% |
| ドキュメント作成 | 5.0 | 60,000 | 300,000 | 4.3% |
| プロジェクト管理 | 3.0 | 65,000 | 195,000 | 2.6% |
| **小計** | **117.0人日** | - | **9,000,000** | **100%** |
| **消費税（10%）** | - | - | **900,000** | - |
| **合計（税込）** | - | - | **9,900,000** | - |

### 単価設定の根拠
- **フロントエンド/バックエンド**: 80,000円/人日
  - React 19 + TypeScript 5の最新技術スタック
  - FastAPI + Python 3.9のモダン開発
  - 複雑な四柱推命ロジック実装
  - シニアレベルの技術力要求

- **テスト実装**: 70,000円/人日
  - Playwright E2Eテスト53項目
  - 100% Pass達成
  - ミドル〜シニアレベル
...
- **デプロイ・インフラ**: 70,000円/人日
  - Vercel + GCP Cloud Run構築
  - 本番環境最適化
  - セキュリティ対応
...
| ページ名 | 工数（人日） | 単価（円） | 金額（円） |
|---------|------------|----------|-----------|
| TopPage（命式記入） | 3.0 | 80,000 | 240,000 |
| ListPage（命式一覧） | 2.5 | 80,000 | 200,000 |
| SajuDetailPage（詳細） | 5.0 | 80,000 | 400,000 |
| LoginPage | 1.5 | 80,000 | 120,000 |
| RegisterPage | 2.0 | 80,000 | 160,000 |
| SettingsPage | 2.0 | 80,000 | 160,000 |
| **小計** | **16.0** | - | **1,280,000** |

#### 2. コンポーネント実装費用（10コンポーネント）

| コンポーネント名 | 工数（人日） | 単価（円） | 金額（円） |
|----------------|------------|----------|-----------|
| GoldenPeppaLoading | 1.0 | 80,000 | 80,000 |
| BottomNavigation | 1.0 | 80,000 | 80,000 |
| SajuCard | 1.5 | 80,000 | 120,000 |
| BasicInfoSection | 0.5 | 80,000 | 40,000 |
| PillarsSection | 1.0 | 80,000 | 80,000 |
| LifeGraphSection | 3.0 | 80,000 | 240,000 |
| DaeunScrollSection | 2.0 | 80,000 | 160,000 |
| YearFortuneScrollSection | 1.5 | 80,000 | 120,000 |
| MonthFortuneScrollSection | 1.5 | 80,000 | 120,000 |
| DayFortuneScrollSection | 1.5 | 80,000 | 120,000 |
| **小計** | **14.5** | - | **1,160,000** |

#### 3. API実装費用（14エンドポイント）

| エンドポイント | 工数（人日） | 単価（円） | 金額（円） |
|--------------|------------|----------|-----------|
| POST /api/saju/calculate | 3.0 | 80,000 | 240,000 |
| POST /api/saju/save | 1.0 | 80,000 | 80,000 |
| GET /api/saju/list | 1.0 | 80,000 | 80,000 |
| GET /api/saju/{id} | 0.5 | 80,000 | 40,000 |
| DELETE /api/saju/{id} | 0.5 | 80,000 | 40,000 |
| GET /api/saju/{id}/daeun | 1.5 | 80,000 | 120,000 |
| GET /api/saju/{id}/year/{year} | 1.5 | 80,000 | 120,000 |
| GET /api/saju/{id}/month/{year} | 1.5 | 80,000 | 120,000 |
| GET /api/saju/{id}/day/{year}/{month} | 1.5 | 80,000 | 120,000 |
| GET /api/saju/export | 1.0 | 80,000 | 80,000 |
| POST /api/saju/migrate | 1.5 | 80,000 | 120,000 |
| POST /api/auth/login | 1.5 | 80,000 | 120,000 |
| POST /api/auth/register | 1.5 | 80,000 | 120,000 |
| POST /api/auth/refresh | 1.0 | 80,000 | 80,000 |
| **小計** | **19.0** | - | **1,520,000** |

#### 4. コアロジック実装費用

| モジュール | 工数（人日） | 単価（円） | 金額（円） |
|-----------|------------|----------|-----------|
| 四柱推命計算（lunar-python統合） | 4.0 | 80,000 | 320,000 |
| ドンサゴン分析（マトリックス） | 5.0 | 80,000 | 400,000 |
| 吉凶判定（7段階システム） | 2.5 | 80,000 | 200,000 |
| 大運計算（10年周期） | 2.0 | 80,000 | 160,000 |
| 人生グラフデータ生成 | 2.0 | 80,000 | 160,000 |
| 210年節気DB統合 | 1.5 | 80,000 | 120,000 |
| **小計** | **17.0** | - | **1,360,000** |

#### 5. テスト実装費用（53項目）

| テストチェーン | 項目数 | 工数（人日） | 単価（円） | 金額（円） |
|--------------|--------|------------|----------|-----------|
| CHAIN-001: 命式計算全体フロー | 11 | 2.5 | 70,000 | 175,000 |
| CHAIN-002: 水平スクロール運勢表示 | 4 | 1.0 | 70,000 | 70,000 |
| CHAIN-003: インタラクティブ運勢選択 | 5 | 1.5 | 70,000 | 105,000 |
| CHAIN-004: ゲスト→ログイン移行 | 5 | 2.0 | 70,000 | 140,000 |
| CHAIN-005: データ管理統合 | 7 | 2.0 | 70,000 | 140,000 |
| CHAIN-006: レスポンシブ対応 | 20 | 3.0 | 70,000 | 210,000 |
| UI改善テスト | 7 | 1.0 | 70,000 | 70,000 |
| **小計** | **53** | **13.0** | - | **910,000** |
...
| 項目 | 工数（人日） | 単価（円） | 金額（円） |
|------|------------|----------|-----------|
| デザインシステム統一 | 1.5 | 75,000 | 112,500 |
| 認証システム（JWT） | 2.0 | 80,000 | 160,000 |
| レスポンシブ対応 | 3.0 | 75,000 | 225,000 |
| Vercelデプロイ | 1.0 | 70,000 | 70,000 |
| GCP Cloud Runデプロイ | 2.0 | 70,000 | 140,000 |
| ドキュメント作成 | 5.0 | 60,000 | 300,000 |
| プロジェクト管理 | 3.0 | 65,000 | 195,000 |
| **小計** | **17.5** | - | **1,202,500** |
...
| 項目 | 金額（円） |
|------|-----------|
| ページ実装（6ページ） | 1,280,000 |
| コンポーネント実装（10個） | 1,160,000 |
| API実装（14エンドポイント） | 1,520,000 |
| コアロジック実装 | 1,360,000 |
| テスト実装（53項目） | 910,000 |
| その他（デザイン・デプロイ等） | 1,202,500 |
| **小計（税抜）** | **7,432,500** |
| **調整額** | **1,567,500** |
| **開発費用合計（税抜）** | **9,000,000** |
| **消費税（10%）** | **900,000** |
...
- **コストパフォーマンス**: 従来の半分以下の期間で高品質実装を実現
