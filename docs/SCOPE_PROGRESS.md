...
最終更新: 2025-11-03（最終検証完了）

---

## E2Eテスト分析レポート - E2E-CHAIN-003-S1

### 基本情報
- テストID: E2E-CHAIN-003-S1
- 対象ページ: /detail/{id} (命式詳細ページ)
- 実行回数: 3回(1回目: 失敗、2回目: 部分成功、3回目: 失敗)
- 最終実行日時: 2025-11-03 14:55
...
Location: /Users/shiraishiami/Desktop/Bluelamp/donsagong-master/frontend/tests/e2e/chain-003-interactive-fortune.spec.ts:91:35

待機対象: GET /api/saju/saju-1c2b6424-55e8-4ea4-bdbf-22e46f66f454/year/18
```

#### ブラウザコンソールログ
```
命式ID: saju-1c2b6424-55e8-4ea4-bdbf-22e46f66f454
```

#### ネットワークログ
```
API直接呼び出し確認:
curl http://localhost:8432/api/saju/saju-1c2b6424-55e8-4ea4-bdbf-22e46f66f454/year/18
→ {"detail":"命式が見つかりません"}

命式ID: saju-1c2b6424-55e8-4ea4-bdbf-22e46f66f454 (LocalStorageのゲストデータID)
バックエンドはこのIDを認識できない（ゲストデータのため）
```

#### ページスナップショット（エラー時）
```yaml
画面表示状態:
  - 基本情報: 正常表示（テスト太郎（CHAIN-003）、1990年3月15日 14:30）
  - 命式（四柱）: 正常表示（庚午/己卯/己卯/辛未）
  - 人生グラフ: 正常表示（8-97歳）
  - 今日の運: 正常表示（2025年、11月、3日）
  - 大運: 正常表示（9個の大運カード）
  - 年運（28-37歳）: 自動表示（現在の大運、10個の年運カード）
  - 月運（2025年）: 自動表示（12個の月運カード）

問題:
  - 18-27歳の大運をクリックしたが、年運APIが呼ばれていない
  - 画面には28-37歳の年運が表示されている（自動表示、クリック前から）
```

#### バックエンドログ（最新100行）
```
ログファイルが存在しません
/Users/shiraishiami/Desktop/Bluelamp/donsagong-master/backend/main.py: No such file or directory
```

#### スクリーンショット
- 保存先: `test-results/chain-003-interactive-fort-46f9a-IN-003-S1-正常系---大運クリックで年運表示-chromium/test-failed-1.png`
- 画面状態: 命式詳細ページ表示、四柱・大運・年運が正常表示

#### 環境情報
- フロントエンドサーバー: 起動中 (localhost:3247)
- バックエンドサーバー: 起動中 (localhost:8432)
- Node.js: v24.5.0
- Playwright: v1.56.1

#### 環境変数詳細（Fail時のみ）
- フロントエンド環境変数: 空（環境変数未設定、テストには影響なし）
- バックエンド環境変数: 未確認

#### 依存関係（エラー内容による）
- npm依存関係: 未確認（モジュールエラーなし）
- Python依存関係: 未確認（必要時のみ）

#### 問題の状況
- 命式IDは `saju-1c2b6424-55e8-4ea4-bdbf-22e46f66f454`（LocalStorageのゲストデータID）
- バックエンド年運API (`/api/saju/{id}/year/{daeun_start_age}`) は実装済み
- しかし、ゲストデータのIDではバックエンドAPIを呼び出せない（「命式が見つかりません」）
- 画面には年運が表示されているが、これは現在の大運（28-37歳）の年運が自動表示されているため
- テストは「18-27歳の大運をクリック→年運API呼び出し」を期待しているが、APIが呼ばれていない
...
## E2Eテスト分析レポート - E2E-CHAIN-005-S3

### 基本情報
- テストID: E2E-CHAIN-005-S3
- 対象ページ: /settings
- 実行回数: 1回（失敗）
- 実行日時: 2025-11-03 13:49
...
Received:   0
  ...
  210 |   expect(exportData).toHaveProperty('count');
  211 |   expect(exportData).toHaveProperty('data');
> 212 |   expect(exportData.count).toBeGreaterThan(0);
      |                            ^
  213 |   expect(exportData.data).toBeInstanceOf(Array);
  214 |   expect(exportData.data.length).toBeGreaterThan(0);
  215 |   expect(exportData.data[0]).toHaveProperty('id');

Location: /Users/shiraishiami/Desktop/Bluelamp/donsagong-master/frontend/tests/e2e/chain-005-data-management.spec.ts:212:28
```
...
Browser console: [LoginPage] handleLogin called {email: test@example.com, password: ***}
Browser console: [LoginPage] Calling login function...
Browser console: [AuthContext] login called {email: test@example.com}
Browser console: [AuthContext] Calling authService.login...
Browser console: [authService] login called {email: test@example.com}
Browser console: [authService] Sending POST /api/auth/login
Browser console: [authService] Response received {status: 200}
Browser console: [authService] Login success, token saved
Browser console: [AuthContext] Login response received
Browser console: [LoginPage] Login success
Browser console: [vite] connecting...
Browser console: [vite] connected.
Browser console: %cDownload the React DevTools for a better development experience: https://react.dev/link/react-devtools font-weight:bold
```
...
#### ネットワークログ
```
Response: http://localhost:8432/api/auth/me 200
Response: http://localhost:8432/api/saju/export 200

エクスポートAPI正常応答: 200 OK
レスポンスデータ: count=0, data=[] (空配列)
```

#### バックエンドログ（最新100行）
```
ログファイルが存在しません
/Users/shiraishiami/Desktop/Bluelamp/donsagong-master/backend/logs/app.log: No such file or directory
```

#### スクリーンショット
- 保存先: `test-results/chain-005-data-management-E2E-CHAIN-005-S3-正常系---データエクスポート-chromium/test-failed-1.png`
- 画面状態: 設定ページ表示、エクスポートボタンクリック後

#### 環境情報
- フロントエンドサーバー: 起動中 (localhost:3247)
- バックエンドサーバー: 起動中 (localhost:8432)
- DATABASE_URL: 未確認（環境変数リスト空）
- Node.js: v24.5.0
- Playwright: v1.56.1

#### 環境変数詳細（Fail時のみ）
- フロントエンド環境変数: 未確認
- バックエンド環境変数: []（空配列、環境変数が読み込まれていない可能性）
...
#### 問題の状況
- エクスポートAPI (`/api/saju/export`) は200 OKを返しているが、count=0、data=[]の空データが返される
- ログイン成功：test@example.com
- E2E-CHAIN-005-S2で1件削除されたため、現在のデータ件数が0件となっている可能性が高い
...
**最終テスト実行: 2025-11-03 15:25 最終検証完了**
